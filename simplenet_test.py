...
def export_as_image(anomaly_map: torch.Tensor):
    ...
    arr = np.maximum(0, arr - 0.5) # values below 0.5 are not considered anomalies due to the loss function definition.
    arr = np.minimum(255, arr * 1000).astype(np.uint8) # The arr values are scaled up for visualization
